...
function readAuthSql(name: string): string {
  return readFileSync(resolve(__dirname, '../../../auth_service/sql', name), 'utf8')
}

const tenantRoutes: FastifyPluginAsync = async (app) => {
  app.post('/tenants/onboard', { config: { requireTenant: false } }, async (request, reply) => {
    ...
    try {
      ...
      const authSql = readAuthSql('tenant.sql')
      ...
      return {
        ...
        schema_name: tenantRes.rows[0].schema_name,
        status: 'created'
      }
    } catch (err) {
